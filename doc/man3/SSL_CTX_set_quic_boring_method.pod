...
SSL_QUIC_BORING_METHOD,
OSSL_QUIC_BORING_ENCRYPTION_LEVEL,
SSL_CTX_set_quic_boring_method,
SSL_set_quic_boring_method,
SSL_set_quic_boring_transport_params,
SSL_get_peer_quic_boring_transport_params,
SSL_quic_boring_max_handshake_flight_len,
SSL_quic_boring_read_level,
SSL_quic_boring_write_level,
SSL_provide_quic_boring_data,
SSL_process_quic_boring_post_handshake,
SSL_is_quic_boring
- QUIC support
...
=head1 SYNOPSIS
 ...
 typedef enum ssl_quic_boring_encryption_level_t OSSL_QUIC_BORING_ENCRYPTION_LEVEL;
 ...
 int SSL_set_quic_boring_transport_params(SSL *ssl,
...
SSL_CTX_set_quic_boring_method() and SSL_set_quic_boring_method() configures the QUIC methods.
This should only be configured with a minimum version of TLS 1.3. B<quic_boring_method>
...
SSL_set_quic_boring_transport_params() configures B<ssl> to send B<params> (of length
...
SSL_get_peer_quic_boring_transport_params() provides the caller with the value of the
...
SSL_quic_boring_max_handshake_flight_len() returns the maximum number of bytes
that may be received at the given encryption level. This function should be
used to limit buffering in the QUIC implementation.

See https://tools.ietf.org/html/draft-ietf-quic-transport-16#section-4.4.

SSL_quic_boring_read_level() returns the current read encryption level.

SSL_quic_boring_write_level() returns the current write encryption level.

SSL_provide_quic_boring_data() provides data from QUIC at a particular encryption
...
SSL_process_quic_boring_post_handshake() processes any data that QUIC has provided
after the handshake has completed. This includes NewSessionTicket messages
sent by the server.

SSL_is_quic_boring() indicates whether a connection uses QUIC.
...
B<SSL_QUIC_BORING_METHOD> to configure secrets and send data. If data is needed from
the peer, it will return B<SSL_ERROR_WANT_READ>. When received, the caller
should call SSL_provide_quic_boring_data() and then SSL_do_handshake() to continue
the handshake. After the handshake is complete, the caller should call
SSL_provide_quic_boring_data() for any post-handshake data, followed by
SSL_process_quic_boring_post_handshake() to process it. It is an error to call
...
level is active in TLS. Callers should use SSL_quic_boring_read_level() to determine
the active read level for SSL_provide_quic_boring_data(). SSL_do_handshake() will
pass the active write level to add_handshake_data() when writing data. Callers
can use SSL_quic_boring_write_level() to query the active write level when
...
SSL_quic_boring_max_handshake_flight_len() to get the maximum buffer length at each
encryption level.
...
=head2 OSSL_QUIC_BORING_ENCRYPTION_LEVEL

B<OSSL_QUIC_BORING_ENCRYPTION_LEVEL> (B<enum ssl_quic_boring_encryption_level_t>) represents the
encryption levels:

=over 4

=item ssl_quic_boring_encryption_initial

The initial encryption level that is used for client and server hellos.

=item ssl_quic_boring_encryption_early_data

The encryption level for early data. This is a write-level for the client
and a read-level for the server.

=item ssl_quic_boring_encryption_handshake

The encryption level for the remainder of the handshake.

=item ssl_quic_boring_encryption_application

The encryption level for the application data.

=back

=head2 SSL_QUIC_BORING_METHOD

The B<SSL_QUIC_BORING_METHOD> (B<struct ssl_quic_boring_method_st>) describes the
QUIC methods.
 ...
 struct ssl_quic_boring_method_st {
     ...
     int (*send_alert)(SSL *ssl, enum ssl_quic_boring_encryption_level_t level, uint8_t alert);
 };
 typedef struct ssl_quic_boring_method_st SSL_QUIC_BORING_METHOD;
...
level other than B<ssl_quic_boring_encryption_initial> is used. Note, however, that
...
together. The exception is B<ssl_quic_boring_encryption_early_data>, where secrets are
only available in the client to server direction. The other secret will be
NULL. The server acknowledges such data at B<ssl_quic_boring_encryption_application>,
...
SSL_CTX_set_quic_boring_method(),
SSL_set_quic_boring_method(),
SSL_set_quic_boring_transport_params(), and
SSL_process_quic_boring_post_handshake()
return 1 on success, and 0 on error.

SSL_quic_boring_read_level() and SSL_quic_boring_write_level() return the current
encryption  level as B<OSSL_QUIC_BORING_ENCRYPTION_LEVEL> (B<enum ssl_quic_boring_encryption_level_t>).

SSL_quic_boring_max_handshake_flight_len() returns the maximum length of a flight
for a given encryption level.

SSL_is_quic_boring() returns 1 if QUIC is being used, 0 if not.
