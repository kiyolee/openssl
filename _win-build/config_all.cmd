...
set OPENSSL_VER=3.0.0-dev
set OPENSSL_VER_SED=3\.0\.0-dev
...
set _GEN_LIST=^
  ...
  %_GEN_LIST_PROV_INCL% ^
  %_GEN_LIST_PROV_CSRC% ^
  apps\progs.c apps\progs.h apps\CA.pl
...
for %%f in ( %_GEN_LIST_PROV_INCL% ) do (
  perl -I. -I%OPENSSL_DIR%\providers\common\der -Mconfigdata -Moids_to_c %OPENSSL_DIR%\util\dofile.pl -omakefile %OPENSSL_DIR%\providers\common\der\%%~nxf.in > %%f
)
for %%f in ( %_GEN_LIST_PROV_CSRC% ) do (
...
dos2unix %_GEN_LIST% apps\tsget.pl tools\c_rehash.pl
exit /b
