 ...
 */
/*
 * Generated from argon2.c.in for https://github.com/kiyolee/openssl.git.
 */
        ...
        for (; (s = p->key) != NULL; p++)
            ...
            case 'a':
                if (ossl_likely(strcmp("d", s + 1) == 0)) {
                    if (ossl_likely(r->ad == NULL))
                        r->ad = (OSSL_PARAM *)p;
                }
                break;
            case 'e':
                if (ossl_likely(strcmp("arly_clean", s + 1) == 0)) {
                    if (ossl_likely(r->eclean == NULL))
