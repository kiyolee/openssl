 ...
 */
/*
 * Generated from ssl.h.in for https://github.com/kiyolee/openssl.git.
 */
 ...
 *
 * ssl_quic_boring_encryption_level_t represents a specific QUIC encryption level used to
 * transmit handshake messages. BoringSSL has this as an 'enum'.
 */
typedef enum ssl_quic_boring_encryption_level_t {
    ssl_quic_boring_encryption_initial = 0,
    ssl_quic_boring_encryption_early_data,
    ssl_quic_boring_encryption_handshake,
    ssl_quic_boring_encryption_application
} OSSL_QUIC_BORING_ENCRYPTION_LEVEL;

struct ssl_quic_boring_method_st {
    int (*set_encryption_secrets)(SSL *ssl, OSSL_QUIC_BORING_ENCRYPTION_LEVEL level,
                                  const uint8_t *read_secret,
                                  const uint8_t *write_secret, size_t secret_len);
    int (*add_handshake_data)(SSL *ssl, OSSL_QUIC_BORING_ENCRYPTION_LEVEL level,
                              const uint8_t *data, size_t len);
    int (*flush_flight)(SSL *ssl);
    int (*send_alert)(SSL *ssl, OSSL_QUIC_BORING_ENCRYPTION_LEVEL level, uint8_t alert);
};

__owur int SSL_CTX_set_quic_boring_method(SSL_CTX *ctx, const SSL_QUIC_BORING_METHOD *quic_boring_method);
__owur int SSL_set_quic_boring_method(SSL *ssl, const SSL_QUIC_BORING_METHOD *quic_boring_method);
__owur int SSL_set_quic_boring_transport_params(SSL *ssl,
                                                const uint8_t *params,
                                                size_t params_len);
void SSL_get_peer_quic_boring_transport_params(const SSL *ssl,
                                               const uint8_t **out_params,
                                               size_t *out_params_len);
__owur size_t SSL_quic_boring_max_handshake_flight_len(const SSL *ssl, OSSL_QUIC_BORING_ENCRYPTION_LEVEL level);
__owur OSSL_QUIC_BORING_ENCRYPTION_LEVEL SSL_quic_boring_read_level(const SSL *ssl);
__owur OSSL_QUIC_BORING_ENCRYPTION_LEVEL SSL_quic_boring_write_level(const SSL *ssl);
__owur int SSL_provide_quic_boring_data(SSL *ssl, OSSL_QUIC_BORING_ENCRYPTION_LEVEL level,
                                        const uint8_t *data, size_t len);
__owur int SSL_process_quic_boring_post_handshake(SSL *ssl);

__owur int SSL_is_quic_boring(const SSL *ssl);
